...
import type { UpdateFileMutationRequest, UpdateFileMutationResponse, UpdateFilePathParams, UpdateFileQueryParams, UpdateFile400, UpdateFile404 } from "../models/UpdateFile";
...
export async function updateFile(fileType: UpdateFilePathParams["fileType"], fileName: UpdateFilePathParams["fileName"], data: UpdateFileMutationRequest, params?: UpdateFileQueryParams, config: Partial<RequestConfig<UpdateFileMutationRequest>> & { client?: Client } = {}) {
  ...
  const res = await request<UpdateFileMutationResponse, ResponseErrorConfig<UpdateFile400 | UpdateFile404>, UpdateFileMutationRequest>({ method : "PATCH", url : getUpdateFileUrl(fileType, fileName).url.toString(), params, data : formData as FormData, ... requestConfig })
  return res.data
}